...
--
-- This script creates the full dashboard-manager DB configuration for:
--   1. Component: food_poisoning_hospitals
--   2. Card preview: TextUnitChart capacity summary
--   3. Map layer: medical cross symbol map
--   4. Query config for metrotaipei
--   5. Dashboard attachment: map-layers-metrotaipei
--
-- Required static frontend file:
--   Taipei-City-Dashboard-FE/public/mapData/food_poisoning_hospitals_metrotaipei.geojson
--
-- Required frontend symbol support:
--   - Taipei-City-Dashboard-FE/src/store/mapStore.js must preload cross_bold/cross_normal
--   - Taipei-City-Dashboard-FE/src/assets/configs/mapbox/mapConfig.js must define symbol-cross_bold
--
-- IMPORTANT for Windows/PowerShell:
-- Do not pipe this SQL through Get-Content into docker exec, because Chinese text
-- may be converted into question marks before reaching psql. Prefer:
--
--   docker cp db-sample-data/add-food-poisoning-hospitals-component.sql postgres-manager:/tmp/add-food-poisoning-hospitals-component.sql
--   docker exec postgres-manager psql -U postgres -d dashboardmanager -v ON_ERROR_STOP=1 -f /tmp/add-food-poisoning-hospitals-component.sql
--
